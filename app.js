...
function sectionTitle(s){ return state.lang==='ru'?s.ru_title:L(s.uz_title); }
function sectionIntro(s){ return state.lang==='ru'?s.ru_intro:L(s.uz_intro); }
function subName(x){ return state.lang==='ru'?x.ru_name:L(x.uz_name); }
function subMain(x){ return state.lang==='ru'?x.ru_main:L(x.uz_main); }
function subRisk(x){ return state.lang==='ru'?x.ru_risk:L(x.uz_risk); }
function subAction(x){ return state.lang==='ru'?x.ru_action:L(x.uz_action); }
...
const stems={
uz:[
 ['main','{topic} mavzusida asosiy talab nimadan iborat?'],['risk','{topic} bo‘yicha qaysi xato eng xavfli?'],['action','{topic} bo‘yicha xodimning to‘g‘ri harakati qaysi?'],['terms','{topic} uchun qaysi asl terminlar muhim?'],['main','{section} bo‘limida {topic} nima uchun o‘rganiladi?'],['action','Ishlab chiqarishda {topic} bo‘yicha qaror qabul qilishdan oldin nima qilinadi?'],['risk','Qaysi javob {topic} bo‘yicha noto‘g‘ri ish yuritishni bildiradi?'],['main','{topic} bo‘yicha xodim nimani aniq tushunishi kerak?'],['action','Nazoratda {topic} bo‘yicha mos kelmaslik aniqlansa nima qilinadi?'],['terms','{topic} mavzusi qaysi terminlar bilan bog‘liq?'],['main','{section} bo‘limidagi {topic} qaysi jarayonni tushuntiradi?'],['action','{topic} bo‘yicha texnologik intizom qanday saqlanadi?']
],
ru:[
 ['main','Какое основное требование относится к теме «{topic}»?'],['risk','Какая ошибка наиболее опасна по теме «{topic}»?'],['action','Какое действие работника является правильным по теме «{topic}»?'],['terms','Какие исходные термины важны для темы «{topic}»?'],['main','Зачем в разделе «{section}» изучается тема «{topic}»?'],['action','Что нужно сделать перед принятием производственного решения по теме «{topic}»?'],['risk','Какой вариант означает неправильную работу по теме «{topic}»?'],['main','Что работник должен четко понимать по теме «{topic}»?'],['action','Что делать при несоответствии по теме «{topic}»?'],['terms','С какими терминами связана тема «{topic}»?'],['main','Какой процесс раскрывает тема «{topic}» в данном разделе?'],['action','Как обеспечивается технологическая дисциплина по теме «{topic}»?']
]};
